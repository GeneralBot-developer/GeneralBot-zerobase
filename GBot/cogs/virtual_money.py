...
class VMoney(commands.Cog):
    ...
    async def virtualmoney_add(self, ctx, member: nextcord.Member, amount: int):
    ...
    async def get(self, ctx, user: nextcord.Member = None):
        vmoney = await VirtualMoney(ctx.guild.id).get()
        if user is None:
    ...
    @virtualmoney.command(name="hand_over", aliases=["ho"], help="仮想通貨を他のユーザーに渡します。")
    async def hand_over(self, ctx, user, amount):
        ...
        embed = nextcord.Embed(
            title="通貨の受け渡し",
            description=f"{ctx.author.name}さんから{user.name}さんに{amount}{vmoney.unit}を受け渡しました。",
            )
        await ctx.reply(embed=embed)

    @virtualmoney.command(name="store_create", aliases=["sc"], help="物品を販売できます")
    async def store_create(self, ctx, name, params: nextcord.Role, amount):
        vmoney = await VirtualMoney(ctx.guild.id).get()
        if not vmoney:
            return await ctx.reply("仮想通貨が作成されていません。")
    ...
    async def buy(self, ctx, name, amount):
        ...
        await VirtualMoney(ctx.guild.id).set(members=vmoney.members, stores=vmoney.stores)
        embed = nextcord.Embed(
            title="購入",
            description=f"{ctx.author.name}さんが{name}の店に{amount}{vmoney.unit}を購入しました。",
            )
        await ctx.reply(embed=embed)
