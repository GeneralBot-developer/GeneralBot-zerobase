import nextcord
from nextcord.ext.commands import Cog, command, Context, has_guild_permissions, group
import asyncio
...
class moderation(Cog):
    ...
    async def kick(self, ctx, user: nextcord.User, *, reason: str = None):
    ...
    @automoderation.command(name="setting", aliases=["set"], help="自動モデレーションを有効化、または無効化します")
    ...
    @automoderation.command(name="ignore_channel", aliases=["ignore"], help="自動モデレーションを除外するチャンネルを設定します")
    ...
    @automoderation.command(name="ignore_role", aliases=["ignore"], help="自動モデレーションを除外するロールを設定します")
    ...
    @automoderation.command(name="ignore_user", aliases=["ignore"], help="自動モデレーションを除外するユーザーを設定します")
    ...
    async def delete_duplicate_message(self, message):
        ...
        if guild.automoderation:
            ...
            if message.author.id in guild.automoderation_ignore_roles:
                return
            async for msg in message.channel.history(limit=guild.message_delete_limit):
